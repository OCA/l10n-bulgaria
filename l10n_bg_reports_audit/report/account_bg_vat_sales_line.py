...
from ..models.account_move import get_doc_type
...
class AccountBGInfoSaleLine(models.Model):
    ...
    info_tag_5 = fields.Char(
        string="[02-05] Document number",
        help="Controls and rules: no",
        readonly=True,
    )
    info_tag_6 = fields.Date(
        string="[02-06] Document date",
        help="Controls and rules: no",
        readonly=True,
    )
    info_tag_7 = fields.Char(
...
{'GROUP BY ' + self._group() or ''}"""
    ...
    SUM(CASE
            WHEN am.state = 'cancel' THEN 0.00
            WHEN aat.tag_name = 16 AND aat.negate THEN aml.balance
    ...
    SUM(CASE
            WHEN am.state = 'cancel' THEN 0.00
            WHEN aat.tag_name = 18 AND aat.negate THEN aml.balance
            WHEN aat.tag_name = 18 AND NOT aat.negate THEN aml.balance
            ELSE 0.00
            END)*-1 AS account_tag_18,
    SUM(CASE
            WHEN am.state = 'cancel' THEN 0.00
            WHEN aat.tag_name = 19 AND aat.negate THEN aml.balance
